@section('title', 'Admin User Applied details')
                    ...
                    <div class="card-bod">
                        <div class="row">
                            ...
                            <div class="col-md-2 mt-2">
                                <img src="{{ asset('studentImage/'.$users->image) }}"
...
<script>
    $(document).ready(function() {
        ...
        $("#blockUserButton").click(function() {
            var userId = "{{ $users->id }}"; // Replace with the actual user ID
            // Send an AJAX request to your Laravel route to block the user
            $.ajax({
                url: "{{ route('block.user', ['id' => $users->id]) }}", // Replace with your actual route
                type: "POST",
                data: {
        ...
        $("#unblockUserButton").click(function() {
            ...
            $.ajax({
                url: "{{ route('unblockUser.user', ['id' => $users->id]) }}", // Replace with your actual route
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}", // Add the CSRF token
                },
